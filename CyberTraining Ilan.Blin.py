...
def supprimer_verbe():
    ...
    infinitif = input("Quel verbe veux-tu supprimer ? (ex: be) : ").strip().lower()
    ...
    con = sqlite3.connect('words.db')
    cursor = con.cursor()
    cursor.execute('DELETE FROM verbes WHERE LOWER(infinitif) = ?', (infinitif,))
...
def menu_quiz():
    while True:
        ...
        choix = input("Choisis une option (1-6) : ")
